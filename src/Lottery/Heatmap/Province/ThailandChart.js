...
const Item = ({atitlenames, url, btime, won2, news}) => (
        ...
        <Sidebar show={showDetail}>
            {showDetail && 
                ...
                <div>
                    ...
                    <h1 style={{textAlign: 'center', marginTop: '-1em'}}>{province_count+' ข่าว'}</h1>
                    <h4 style={{textAlign: 'center', fontWeight: 400,  marginTop: '-2em'}}>{'(อันดับ '+rank_text+')'}</h4>
                </div>
                <div>
                    <SidebarLineChart values={value_lot_date} dates={all_lot_date} baseValues={base_value_lot_date} width={'300px'} height={'300px'} scale={10}/>
                </div>
                {news_index===-1 && <div style={{display:'flex', 'flew-direction': 'row'}}>
                    ...
                    <div>
                        <SidebarBarChart values={news_count} width={'300px'} height={'190px'}/>
                    </div>
                </div>}
...
export class ThailandChart extends Component {
  constructor(props){
      super();
      let news_index = -1
      let { data } = props;
      let { values } = prepareData(data, news_index)
      this.state = {
          mapData: values,
          showDetail: false,
  ...
  componentWillUpdate(nextProps, nextState){
      if(nextState.news_index !== this.state.news_index){
          let { data } = nextProps;
          let { values } = prepareData(data, nextState.news_index)
          nextState.mapData = values
          showThailandMap(nextState.mapData, this.showSideBar, this.hideSideBar)
      }
  }
  async componentDidMount(){
      const { mapData } = this.state;
      showThailandMap(mapData, this.showSideBar, this.hideSideBar)
  }
  showSideBar = (province) => {
      ...
      let province_count = news_index === -1 ? 0 : data.values[news_index].province_count[index_pro]
      ...
      if(news_index !== -1){
          let pc = data.values[news_index].province_count
          pc = _.orderBy(pc,null,['desc'])
          rank = pc.indexOf(province_count)+1
      }
      
      let value_lot_date = news_index === -1 ? [] : data.values[news_index].province_lot_date[index_pro]
      ...
      if(news_index === -1 && index_pro !== -1) {
          ...
          province_count += data.values[0].province_count[index_pro]
          province_count += data.values[1].province_count[index_pro]
          province_count += data.values[2].province_count[index_pro]
          province_count += data.values[3].province_count[index_pro]


          for(let i=0;  i < all_lot_date.length;  ++i){
            let value = data.values[0].province_lot_date[index_pro][i]+data.values[1].province_lot_date[index_pro][i]+data.values[2].province_lot_date[index_pro][i]+data.values[3].province_lot_date[index_pro][i]
      ...
      //console.log('posts', posts, province_count, rank, value_lot_date, data.base_value_lot_date)
      this.setState({
          popupData: {
              news_index,
              province,
              ...
              all_lot_date,
              news_count,
              base_value_lot_date: data.base_value_lot_date,
          }
      })
  }
  ...
  render(){
    ...
    return(<Container>
        ...
        <svg id="thailand-news" width="1000" height="500"></svg>
    </Container>)
  }
}