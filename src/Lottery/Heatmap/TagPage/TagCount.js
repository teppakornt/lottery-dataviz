        ...
        <Sidebar show={showDetail}>
            {showDetail && 
                ...
                <div>
                    ...
                    <h1 style={{textAlign: 'center', marginTop: '-1em'}}>{news_count+' ข่าว'}</h1>
                    <h4 style={{textAlign: 'center', fontWeight: 400,  marginTop: '-2em'}}>{'(อันดับ '+rank_text+')'}</h4>
                </div>
                <div>
                    <SidebarLineChart 
                    values={news_overtime} 
                    dates={all_lot_date} 
                    baseValues={base_news_overtime} 
                    maxY={50}
                    width={'300px'} height={'300px'} 
                    lineText={'จำนวนข่าวทั้งประเทศ'}
...
export class TagCount extends Component {
  constructor(props){
      super();
      this.state = {
          news_index: -1,
  ...
  showSideBar = (d) => {
      ...
      //RANK
      let sp = allTagData.map(x=>x.news_count)
      sp = _.orderBy(sp,null,['desc'])
      let rank = sp.indexOf(tagData.news_count)+1;

      //NEWS BAR CHART
      let news_bar_chart = NEWS_CODE.map(x => 0)
      let detail = tagData.detail
  ...
  render(){
    ...
    return(<Container>
        ...
        <WordCloudD3 
            data={data.tag_data[news_index+1].tags.map(x=> ({text:x.tag,  value: news_index > 0 ? x.news_count*2 : x.news_count}))}
