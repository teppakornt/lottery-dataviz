...
export class LeftSidebar extends Component {
    render(){
        ...
        //console.log('won_overtime', won_overtime)
        return(
        <Sidebar show={showDetail}>
            {showDetail && 
                ...
                <div>
                    ...
                    <h1 style={{textAlign: 'center', marginTop: '-1em'}}>{Math.floor(accuracy*10000)/100+' %'}</h1>
                ...
                <div>
                    ...
                    values={won_overtime} 
                    dates={all_lot_date} 
                    baseValues={num2_overtime.map(x => x.length)} 
                    maxY={25}
                    width={'300px'} height={'300px'} 
                    baseLineText={'จำนวนเลขของสิ่งศักดิ์สิทธิ์'}
...
export class TagAccuracy extends Component {
  constructor(props){
      super();
      this.state = {
          news_index: -1,
  ...
  showSideBar = (d) => {
      ...
      //RANK
      let sp = allTagData.map(x=>x.accuracy)
      sp = _.orderBy(sp,null,['desc'])
      let rank = sp.indexOf(tagData.accuracy)+1;
  ...
  render(){
    ...
    return(<Container>
        ...
        <WordCloudD3 
            data={data.tag_data[news_index+1].tags.map(x=> ({text:x.tag,  value: news_index > 0 ? x.accuracy*1000 : x.accuracy*1000}))}
            onClick={this.showSideBar}
            colorRange={['#fff', '#f39c12']}/>
    </Container>)
  }
}
